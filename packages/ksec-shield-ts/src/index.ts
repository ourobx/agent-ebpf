...
export class KsecShield {
  ...
  public async syncPolicies(): Promise<void> {
    try {
  ...
  public async guard<T>(fn: () => Promise<T> | T, options: GuardOptions): Promise<T> {
    ...
    if (evaluation.decision === 'BLOCK') {
      ...
      throw new KsecSecurityViolationError(
        `Execution blocked by Agent-eBPF kernel shield: ${options.actionType} on '${options.target}'`,
        options.actionType,
        options.target,
        evaluation.rule?.id
      );
    ...
    if (!udsHandled && this.config.gatewayUrl) {
      try {
        ...
        if (!remoteRes.allowed || remoteRes.decision === 'BLOCK') {
          ...
          throw new KsecSecurityViolationError(
            `Execution blocked by Agent-eBPF gateway: ${options.actionType} on '${options.target}' (${reason})`,
            options.actionType,
            options.target
          );
        }
        ...
        if (this.config.fallbackPolicy === 'fail-closed') {
          throw new KsecSecurityViolationError(
            `Execution blocked by fail-closed policy (gateway evaluation failed: ${err.message})`,
            options.actionType,
            options.target
          );
        }
   ...
   */
  public protectFetch(originalFetch = globalThis.fetch): typeof globalThis.fetch {
    return async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (typeof input === 'string') {
        ...
        try {
          target = new URL((input as Request).url).hostname;
        } catch {
          target = (input as Request).url;
        }
      }
  ...
  private recordTelemetry(event: ShieldTelemetryEvent): void {
  ...
  public on(event: ShieldEventType, listener: ShieldEventListener): () => void {
  ...
  public emitCustomEvent(event: ShieldEventType, payload: Record<string, unknown>): void {
